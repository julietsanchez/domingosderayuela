...
const SUMATE_SHEET_HEADERS = [
  ...
  '¿Voluntariado previo?',
  '¿Por qué te gustaría ser voluntario/a?',
  '¿Cómo te enteraste?',
...
function describeSheetsFailure(status, body) {
  if (status === 401) {
    return (
      'HTTP 401 — Revisá el despliegue en Apps Script: (1) Ejecutar como: Yo, ' +
      '(2) Acceso: Cualquier usuario / Anyone (no "con cuenta de Google"), ' +
      '(3) URL /exec de la implementación activa en Vercel, (4) probá la URL en incógnito (debe verse JSON ok)'
    );
  }
  if (status === 302 || status === 303) {
    return (
      'HTTP ' +
      status +
      ' — Google redirige al login; el acceso del despliegue no es "Cualquier usuario" o la URL no es la Web App /exec'
    );
  }
