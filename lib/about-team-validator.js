...
function validateAboutTeamPage(html) {
  ...
  teamMemberBlocks.forEach(function(block) {
    ...
    if (/<p class="team-card__role">/.test(block)) {
      errors.push(`${name}: no debe incluir texto descriptivo mientras la card esté simplificada.`);
    }
  ...
  const alumniSectionMatch = html.match(/<section class="section about-alumni-voices"[^>]*>/);
  if (!html.includes('id="ex-voluntarias-title">Voces de ex voluntarias</h2>')) {
    errors.push('Falta el título "Voces de ex voluntarias".');
  }

  if (!alumniSectionMatch || !/\bhidden\b/.test(alumniSectionMatch[0])) {
    errors.push('La sección "Voces de ex voluntarias" debe permanecer oculta con el atributo hidden.');
  }
