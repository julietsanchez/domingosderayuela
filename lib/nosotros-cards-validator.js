...
function validateNosotrosCards(html) {
  ...
  cards.forEach(function(cardHtml, index) {
    ...
    if (isBlurBack) {
      if (miniPhotoCount !== 0) {
        errors.push(`${title}: las cards con fondo difuminado no deben incluir mini fotos.`);
      }

      if (!/--card-img:\s*url\(/.test(cardHtml)) {
        errors.push(`${title}: la cara posterior difuminada debe declarar --card-img.`);
      }
    } else if (miniPhotoCount !== EXPECTED_MINI_PHOTO_COUNT) {
